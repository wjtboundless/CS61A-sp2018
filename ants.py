...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        #Start with current place. Keep looking for bees until the hive. If we find a spot 
        with a bee (or multiple bees), return one of them.  
        """
        # BEGIN Problem 3 and 4
        pos = self.place
        distance = 0
        while pos != hive:
            if pos.bees and self.min_range <= distance < self.max_range:
                return random_or_none(pos.bees)
            pos = pos.entrance
            distance += 1
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true_queen:
            ...
            while pos:
                if pos.ant:
                    if isinstance(pos.ant, BodyguardAnt) and pos.ant.ant and pos.ant.ant not in self.boosted_ants:
                        pos.ant.ant.damage *= 2
                        self.boosted_ants.append(pos.ant.ant)
                    if pos.ant not in self.boosted_ants:
                        pos.ant.damage *= 2
                        self.boosted_ants.append(pos.ant)
                pos = pos.exit
        # END Problem 13
