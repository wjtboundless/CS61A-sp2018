...
def trade(first, second):
    ...
    m, n = 1, 1
    "*** YOUR CODE HERE ***"
    while m < len(first)+1 and n < len(second)+1:
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def use_list(lst):
        if lst is Link.empty:
            ...
            new_str = front + str(lst.first) + mid 
            return new_str + make_to_string(front, mid, back, empty_repr)(lst.rest) + back
    return use_list
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        b = [tree_map(fn, b) for b in t.branches]
        return Tree(fn(t.label), b)
