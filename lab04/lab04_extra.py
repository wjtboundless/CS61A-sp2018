...
def put_piece(board, max_rows, column, player):
    """Puts PLAYER's piece in the bottommost empty spot(last row) in the given column of
    ...
    if get_piece(board, i, column) == '-':
        ...
        while r < max_rows:
            if board[r][column] == '-':
                ...
                break
        board[r - 1] = replace_elem(board[r - 1], column, player)
        return (r - 1, board)
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    if row < max_rows:
        counter = 0
        for j in range(max_cols):      
            if get_piece(board, row, j) != player:
                counter = 0
            else:
                counter += 1
            if counter >= num_connect:
                return True
    return False
