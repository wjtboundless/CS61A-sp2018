...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            if self.place.characters.get(person):
    ...
    def take(self, thing):
        ...
        1
        """
        #self.Already_empty = False
        if type(thing) != str:
            print('Thing should be a string.')
        else:
            if not self.place.things.get(thing):
            #if self.Already_empty or not self.place.things.get(thing):
                print(thing, 'is not here.')
            else:
                #self.Already_empty = True
